...
import casadi          # 用于符号计算和优化问题求解的库
import numpy as np      # 用于数值计算和矩阵操作
import pinocchio as pin  # 用于机器人运动学/动力学计算的库
from pinocchio import casadi as cpin  # Pinocchio的Casadi接口，用于符号化运动学计算               
...
class Kinematics:
    ...
    def createSolver(self):  # 创建逆运动学求解器（核心方法）
        ...
        # 检查 ID 是否有效 (Pinocchio 如果找不到名字，返回的 ID 会等于 frames 的总长度)
        #if self.ee_id >= len(self.model.frames):
        #    print(f"\n[严重错误] 无法在模型中找到名为 '{self.frame_name}' 的 Frame！")
        #    print(f"当前模型中可用的 Frame 名称列表如下 ({len(self.model.frames)} 个):")
        #    print("-" * 30)
        #    # 打印所有可用名称，方便你排查
        #    for i, f in enumerate(self.model.frames):
        #        print(f"ID {i}: {f.name}")
        #    print("-" * 30)
        #    raise ValueError(f"Frame '{self.frame_name}' does not exist in the MJCF/URDF model.")
        ...
        # 总代价函数（加权求和）：位置精度权重最高，其次是平滑性，姿态精度权重较低
        self.opti.minimize(10.0 * self.translational_cost + 0.5*self.rotation_cost + 0.001 * self.regularization_cost + 0.1 * self.smooth_cost)
